...
CBillboard::CBillboard(LAYER_TYPE layer) : CObject(layer)
{
...
//インスタンス生成処理
//---------------------------------------------------------------
CBillboard *CBillboard::Create(D3DXVECTOR3 pos, D3DXVECTOR3 size, const string &texType, const D3DXCOLOR& col)
{
	//インスタンス生成
	CBillboard *pBillboard = new CBillboard;
	pBillboard->m_pos = pos;
	pBillboard->m_size = size;
	pBillboard->m_col = col;
	pBillboard->BindTexture(texType);
	if (pBillboard != NULL)
	{
		pBillboard->Init();
	}
