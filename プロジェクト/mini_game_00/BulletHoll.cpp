...
//マクロ
#define RAND_HIGTH (float(rand() % 100) + 1)	//高さランダム化
	...
	HEIGHT_TYPE Type)
{
	...
	if (m_Height_Type == TYPE_RAND)
	{
		m_pos = D3DXVECTOR3(pos.x + cosf(CIRCLE2) *  fRandDistance, pos.y + RAND_HIGTH + m_Size.y / 2, pos.z + cosf(CIRCLE2) * fRandDistance2);
	}
	else
	{
		m_pos = D3DXVECTOR3(pos.x + cosf(CIRCLE2) *  fRandDistance, pos.y + m_Size.y / 2, pos.z + cosf(CIRCLE2) * fRandDistance2);
	}
	...
	CBasis_Effect::SetPosBill(m_pos1, m_pos2, m_pos3, m_pos4);
	m_nSynthenic = Synthetic;
	return S_OK;
